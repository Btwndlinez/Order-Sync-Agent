...
// Antigravity High-Velocity Refactor: Parallel Intent & Vector Matching

// Supabase Edge Function: analyze-conversation (Antigravity Matcher Edition)
// Analyzes Messenger conversations to detect purchase intent and match products from DB
...
async function callLLM(prompt: string): Promise<string> {
    ...
    const model = Deno.env.get("LLM_MODEL") ?? "gpt-4o-mini"; // Default to fast-path model
...
serve(async (req) => {
    ...
    try {
        ...
        // ANTIGRAVITY TUNING: Early Exit Protocol
        // If no intent detected, return immediately to save DB writes/latency
        ...
        if (body.messenger_id) {
            supabase.from('conversations').upsert({
                ...
                onConflict: 'seller_id,messenger_id' // Antigravity: Ensure atomic upsert
            }).then(({ error }) => {
        ...
        return new Response(JSON.stringify({ error: (error as Error).message, intent_detected: false }), {
            status: 500,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
    }
